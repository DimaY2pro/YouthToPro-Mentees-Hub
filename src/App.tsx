...
import { auth, loginWithGoogle, logout, registerWithEmail, loginWithEmail } from './lib/firebase';
...
function Home({ user, handleGoogleLogin }: { user: User | null; handleGoogleLogin: () => void }) {
  ...
  const handleAuth = async () => {
    ...
    try {
      if (isLoginMode) {
        ...
        // You could also store firstName/lastName to Firestore here
        navigate('/modules');
      }
    } catch (err: any) {
            ...
            <div className="@container">
              <div className="flex flex-col gap-6 py-10 lg:gap-12 lg:flex-row lg:items-center">
                <div className="flex flex-col gap-6 lg:w-1/2 lg:pr-10">
                  <div className="flex flex-col gap-4 text-left">
                    ...
                    <h1 className="text-primary dark:text-white text-4xl font-black leading-tight tracking-[-0.033em] md:text-5xl lg:text-6xl">
                      Launch Your Career with <span className="text-cta">YouthToPro Hub</span>
                    </h1>
                    <h2 className="text-slate-600 dark:text-slate-300 text-base font-normal leading-relaxed md:text-lg">
                      Connecting ambitious youth with experienced professionals. Join a thriving community where mentorship transforms potential into professional success.
                    </h2>
                  </div>
                  <div className="flex flex-wrap gap-3">
                    <button className="flex cursor-pointer items-center justify-center overflow-hidden rounded-lg h-12 px-6 bg-cta hover:bg-yellow-500 text-primary text-base font-bold leading-normal tracking-[0.015em] transition-all shadow-md hover:shadow-lg">
                      <span className="truncate">Join as Mentee</span>
                    </button>
                    <button disabled className="flex items-center justify-center overflow-hidden rounded-lg h-12 px-6 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 text-base font-bold leading-normal tracking-[0.015em] transition-all opacity-50 cursor-not-allowed" title="Mentor registration is coming soon">
                      <span className="truncate">Join as Mentor</span>
                    </button>
                  </div>
                  <div className="flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400">
                    <div className="flex -space-x-2">
                      <div className="h-8 w-8 rounded-full border-2 border-white dark:border-slate-900 bg-slate-200 bg-center bg-cover" data-alt="Portrait of a smiling young woman" style={{ backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuD4-Fp-MnmPFWEszRk5rGootOw5g4yHjr8BpUy3ASoeBgrZA6UpADhZpxoFn-dO7OdL2IH8DwLztcEzrnY4abQ9uh8nzvOArXhyu0fnKtyA6B2TbAiZqncbOV-pTypd3LS_UriNQ0WdGgyhDtrLpHERxnj-at1rbiqcdELkKizsMgaamvlxOhAoOYoS5KsVpy9tlYRLe-nopG7dsyYZYl5gjSH6DKGGujSeCo6j3vtBFnzH0nFAhkWUTVgHXPPPw-17QyJ8NkDAzp8")' }}></div>
                      <div className="h-8 w-8 rounded-full border-2 border-white dark:border-slate-900 bg-slate-200 bg-center bg-cover" data-alt="Portrait of a young man looking professional" style={{ backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuBlyA7Zg31GSbQZpWB1Z9i0qxxJfnJgfHNED0AmpzluOgvd30I1A1MmWrcBLfMP5MBfQlvBkcZu8N-8AHwaqzeELG_mKJsdJRP_dJv9avItgKSHzIGkf2787PD4i1rXUz8zODV-twkPmjhsC5vgv2EQ_5Qo3Ww_gTfqtDUc6rKdquWrC320RKTRblqvTonM-gv_6QzJGdlNnO-WTSDSCzJKsDN-7H9wDNH6dqZ2b1VtqYGXToCZCeL6ONwzYHqZbb6GN_ViWiaySq4")' }}></div>
                      <div className="h-8 w-8 rounded-full border-2 border-white dark:border-slate-900 bg-slate-200 bg-center bg-cover" data-alt="Portrait of a smiling man with glasses" style={{ backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuBeDaqbjRbAm6BH7oCH2v8dlYmGzEJ-CElTD04m6VJWx0-25ErnrrFkzZROLWvQs3-S7WzulKrNtr0XPSyIbyVTRGtyqVs9FRo5CK2WNCxb-1yYnQqcK46kBpGpHwuIeASqMcQGgUnZhrgQFGHhLkTlz4luTsm05pGQhpE8m4B2xzXy8Eq7bkz2mC0MZCjjwouRijgY-EQaqPss7W9xG8jVupNqq83IvnjGVBgJGMHUMITBsb-KvZQ4Rbex7g7ba0PBC89eFV3izy4")' }}></div>
                    </div>
                ...
                <div className="lg:w-1/2 mt-8 lg:mt-0 relative">
                  ...
                  <div className="absolute -bottom-4 -left-4 w-32 h-32 bg-cta/30 rounded-full blur-2xl"></div>
                  <div className="relative w-full aspect-[4/3] bg-center bg-no-repeat bg-cover rounded-2xl shadow-xl overflow-hidden" data-alt="Homepage Hero Image" style={{ backgroundImage: 'url("/main%20pic.png")' }}>
          ...
          <div className="layout-content-container flex flex-col w-full max-w-[600px] flex-1">
            <div className="bg-white dark:bg-[#15202b] rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
              <div className="px-8 pt-8 pb-4 text-center">
                <h2 className="text-primary dark:text-white text-2xl font-bold leading-tight tracking-[-0.015em] mb-2">{isLoginMode ? 'Welcome Back' : 'Create Your Account'}</h2>
                <p className="text-slate-500 dark:text-slate-400 text-sm">{isLoginMode ? 'Log in to continue your journey.' : 'Join the community to start your journey.'}</p>
              </div>
              <div className="flex px-8 pb-6">
                <div className="flex h-12 flex-1 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800 p-1 w-full relative">
                  ...
                  <label className="flex h-full grow items-center justify-center overflow-hidden rounded-md px-2 text-slate-400 dark:text-slate-500 text-sm font-bold transition-all opacity-50 cursor-not-allowed" title="Mentor registration is coming soon">
              ...
              <form className="flex flex-col gap-5 px-8 pb-8" onSubmit={(e) => { e.preventDefault(); handleAuth(); }}>
                {error && (
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input value={email} onChange={(e) => setEmail(e.target.value)} required className="flex h-11 w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 pl-10 pr-3 text-slate-900 dark:text-white focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-all placeholder:text-slate-400" id="email" placeholder="you@example.com" type="email" />
                    </div>
                  </div>
                  <div className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center justify-center gap-2 mt-4">
                  <span className="text-sm text-slate-600 dark:text-slate-400">{isLoginMode ? "Don't have an account?" : "Already have an account?"}</span>
                  <button onClick={() => setIsLoginMode(!isLoginMode)} type="button" className="text-sm font-bold text-primary dark:text-cta hover:underline">
              ...
              <div className="layout-content-container flex flex-col w-full max-w-[1200px] flex-1">
                <div className="flex items-center justify-between whitespace-nowrap py-3">
                  ...
                  <div className="hidden md:flex flex-1 justify-end gap-8 items-center">
                    ...
                    {user ? (
                      <div className="flex items-center gap-4">
                        <Link to="/modules" className="text-primary dark:text-slate-300 hover:text-accent dark:hover:text-accent text-sm font-bold leading-normal transition-colors mr-2">
                          Modules Dashboard
                        </Link>
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300 hidden lg:block">{user.email}</span>
                        <button onClick={() => { handleLogout(); navigate('/'); }} className="flex min-w-[100px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-6 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-800 dark:text-white text-sm font-bold leading-normal tracking-[0.015em] transition-colors shadow-sm">
                          <span className="truncate">Log Out</span>
                        </button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-4">
                        <button onClick={handleGoogleLogin} className="text-primary dark:text-slate-300 hover:text-accent dark:hover:text-accent text-sm font-medium leading-normal transition-colors">
                          Login
                        </button>
                        <Link to="/" onClick={() => setTimeout(() => document.getElementById('register')?.scrollIntoView({ behavior: 'smooth' }), 100)} className="flex min-w-[100px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-6 bg-cta hover:bg-yellow-500 text-primary text-sm font-bold leading-normal tracking-[0.015em] transition-colors shadow-sm">
                  ...
                  <div className="md:hidden flex items-center">
                    <button className="text-primary dark:text-white p-2">
